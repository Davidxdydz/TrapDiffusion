...
The training data is generated by solving the given system of equations for multiple initial configurations using a numerical solver. For the single occupation, single isotope model, the stepsize is constant and amounts to 50 steps over 2 seconds, as the differential equation is not particularly stiff and adaptive steps would only yield very few data points.  For the multi occupation, multi isotope model, the stepsize is adaptive and the solver yields concentrations for around 100 different timesteps.
